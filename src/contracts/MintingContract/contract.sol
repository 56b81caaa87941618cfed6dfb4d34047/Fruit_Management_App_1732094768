...
contract AirdropClient is IZKPayClient {
    ...
    uint256 public constant AIRDROP_AMOUNT = 100000000000000;
    ...
    function sxtCallback(
        ...
        for (uint256 i = 0; i < queryResult.columns[0].values.length; i++) {
            address payable recipient = payable(abi.decode(queryResult.columns[0].values[i], (address)));
            (bool success,) = recipient.call{value: AIRDROP_AMOUNT}("");
            require(success, "ETH transfer failed");
            emit Airdropped(recipient, AIRDROP_AMOUNT);
        }
    }
